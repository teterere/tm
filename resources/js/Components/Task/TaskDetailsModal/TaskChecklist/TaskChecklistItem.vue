    ...
    <div class="flex -ml-2">
        <div @mouseover="showActionButtons = true"
            ...
            <div class="flex items-center w-full">
                ...
                <input v-if="editStatus" type="text" v-model="form.description" class="block w-full rounded-md bg-white px-1 -ml-1 mr-1 py-0 text-base text-gray-900 placeholder:text-gray-400 sm:text-sm/6 outline-0 border-0 focus:ring-0 focus:border-0 focus:outline-0">
                <label v-else :for="'item-' + item.id" class="text-sm" :class="item.completed ? 'line-through text-gray-400' : 'text-gray-600'">
                    {{ item.description }}
                </label>
            </div>
